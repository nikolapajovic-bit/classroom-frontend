    ...
    const classColumns = useMemo<ColumnDef<SubjectClass>[]>(() => [
        ...
        },
        {
            ...
            cell: ({ getValue }) => {
                ...
                return (
                    <Badge variant={status === 'active' ? 'default' : 'secondary'}>{status ?? "unknown"}</Badge>
                )
            }
        },
        {
        ...
        <Card className="hover:shadow-md transition-shadow">
            ...
            <CardContent className="space-y-2">
                {details.subject.department ? (
                    <>
                        <Link to={`/departments/show/${details.subject.department.id}`} className='text-lg text-foreground font-semibold hover:underline'>
